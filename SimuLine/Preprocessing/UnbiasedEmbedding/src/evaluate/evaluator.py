...
from SimuLine.Preprocessing.UnbiasedEmbedding.src.evaluate.metrics import average_precision_at_k, dcg_at_k, recall_at_k
...
def aoa_evaluator(user_embed: np.ndarray,
    ...
    for user in tqdm(list(set(users))[:1000]):
        indices = users == user
